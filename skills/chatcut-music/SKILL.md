...
3. Call `submit_music`.
4. Unless the user explicitly asked only to queue the job, call `track_progress` with `target:"generation"`, `action:"wait"`, and the returned `jobId`. Desktop keeps that call open reactively so the completed audio asset or failure returns to the same conversation.
