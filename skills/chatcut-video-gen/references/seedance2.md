...
All frame/reference slots take a project **asset id** (UUID or short prefix from `browse_assets`), `asset://<id>`, or a same-project asset URL. External URLs are not accepted — download them to readable local paths on this computer, register them with Desktop `push_asset`, and use the resulting asset ids.
