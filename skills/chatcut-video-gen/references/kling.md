...
Image inputs take a project **asset reference**. Prefer a UUID or short prefix from `browse_assets`; `asset://<id>` and same-project asset URLs returned by asset tools are also accepted. External URLs and base64 are rejected at the tool boundary — import them into the project first.
