...
The current [plan-mode.md](../rules/plan-mode.md) Workflow has a `Rename` rule (§3) that only triggers a rename **when the plan's purpose shifts**:
...
Tighten Workflow §3 in [plan-mode.md](../rules/plan-mode.md) so it requires a meaningful `verb-target` kebab-case filename on **every** plan before commit — covering both (a) initial creation with an auto-generated/placeholder slug and (b) the existing purpose-shifted case.
...
1. **Edit Workflow §3 (`Rename`) in `plan-mode.md`** to require a meaningful filename in two scenarios, not just one: (a) when the initial filename is auto-generated, placeholder, or otherwise non-descriptive, and (b) when the plan's purpose shifts after creation. Reference back to the `verb-target` kebab-case convention already defined in §1 (`Create`) as the authoritative shape, and keep the existing "stale filename is a plan defect — do not commit until the name matches the content" closing clause.
   ...
   - *Verify:* Re-read [plan-mode.md](../rules/plan-mode.md) §3 and confirm it (1) names both the placeholder-name and purpose-shift cases, (2) refers to the §1 `verb-target` kebab-case convention rather than restating it, (3) retains the "stale filename is a plan defect" closing clause.
...
- Open [plan-mode.md](../rules/plan-mode.md) and read Workflow §3. It must name both triggers (placeholder-name on creation, and purpose-shift later), point back to §1's `verb-target` convention, and keep the "stale filename is a plan defect" closing clause.
